...
network <- d %>%
  dplyr::mutate_at(vars(starts_with("network")), ~  
                         as.numeric(ifelse(.x=="-999", 0, .x  ))) %>%
...
save(crime, file=here("data", "network_results_large_ind_net_no_na.Rdata"))

save(ab2, file=here("data", "network_results_all_k_no_na.Rdata"))
...
ggplot(crime %>% mutate(group=str_replace_all(group,"_", " "), 
                        group=str_to_title(str_remove_all(group,"network"))), aes(res)) +
  geom_histogram(alpha=.2, color="black") +
  geom_vline(aes(xintercept=0), color="tomato2", linetype="dashed") +
  facet_wrap(~group) +
  theme_minimal(base_size = 18)

ggsave(filename=here("output", "densities.png") ,
       width = 12, height = 8, units = "in", pointsize = 12, bg = "white")

save(crime, file=here("data", "network_results_all_no_na.Rdata"))
...
stargazer(models$model[[3]], models$model[[1]], models$model[[6]],models$model[[4]], 
          ...
          covariate.labels = c("Intercept", 
                               ...
                               "Crime Victim" , "Police Violence", 
                               "Punitive Preferences", "Fear of Crime", 
                               "Trust in the Police"), 
          model.numbers = FALSE)


levels(model_crime$term)
